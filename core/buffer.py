...
class A2CRolloutStorage:
    def __init__(self, num_steps, num_processes, obs_shape, device):
        ...
        self.action_log_probs_cash = zeros(num_steps, num_processes, 1)
        self.actions_cash = zeros(num_steps, num_processes, 1).to(torch.long)
        self.action_log_probs_beta = zeros(num_steps, num_processes, 1)
        self.actions_beta = zeros(num_steps, num_processes, 1).to(torch.long)
        self.masks = torch.ones(num_steps + 1, num_processes, 1).to(device)

        self.num_steps = num_steps
        self.step = 0

    def insert(self, current_obs, action_cash, action_log_prob_cash, action_beta, action_log_prob_beta, value_pred, reward,
               mask, env_id):
        self.observations[self.step + 1, env_id].copy_(current_obs)
        self.actions_cash[self.step, env_id].copy_(action_cash)
        self.action_log_probs_cash[self.step, env_id].copy_(action_log_prob_cash)
        self.actions_beta[self.step, env_id].copy_(action_beta)
        self.action_log_probs_beta[self.step, env_id].copy_(action_log_prob_beta)
        self.value_preds[self.step, env_id].copy_(value_pred)
        self.rewards[self.step, env_id].copy_(reward)
        self.masks[self.step + 1, env_id].copy_(mask)
...
class PPORolloutStorage(A2CRolloutStorage):
    ...
    def feed_forward_generator(self, advantages, mini_batch_size):
        ...
        for indices in sampler:
            ...
            #     indices]
            actions_cash_batch = self.actions_cash.view(-1, self.actions_cash.size(-1))[
                indices]
            actions_beta_batch = self.actions_beta.view(-1, self.actions_beta.size(-1))[
                indices]
            return_batch = self.returns[:-1].view(-1, 1)[indices]
            masks_batch = self.masks[:-1].view(-1, 1)[indices]
            old_action_log_probs_cash_batch = self.action_log_probs_cash.view(-1, 1)[
                indices]
            old_action_log_probs_beta_batch = self.action_log_probs_beta.view(-1, 1)[
                indices]
            adv_targ = advantages.view(-1, 1)[indices]

            yield observations_batch, actions_cash_batch, actions_beta_batch, return_batch, \
                  masks_batch, old_action_log_probs_cash_batch, old_action_log_probs_beta_batch, adv_targ
    ...
    def compute_returns(self, next_value, gamma):
        if self.gae:
            ...
            for step in reversed(range(self.rewards.size(0))):
                ...
                delta = self.value_preds[step+1]*gamma*self.masks[step+1]+self.rewards[step]-self.value_preds[step]
                gae = self.gae_lambda*gamma*gae*self.masks[step+1] + delta
