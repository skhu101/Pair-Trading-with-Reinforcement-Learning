...
class PPOTrainer(BaseTrainer):
    ...
    def compute_loss(self, sample):
        ...
        observations_batch, actions_cash_batch, actions_beta_batch, return_batch, masks_batch, \
        old_action_log_probs_cash_batch, old_action_log_probs_beta_batch, adv_targ = sample

        assert old_action_log_probs_cash_batch.shape == (self.mini_batch_size, 1)
        assert old_action_log_probs_beta_batch.shape == (self.mini_batch_size, 1)
        assert adv_targ.shape == (self.mini_batch_size, 1)
        assert return_batch.shape == (self.mini_batch_size, 1)

        values, action_log_probs_cash, action_log_probs_beta, dist_entropy = self.evaluate_actions(
            observations_batch, actions_cash_batch, actions_beta_batch)

        assert values.shape == (self.mini_batch_size, 1)
        assert action_log_probs_cash.shape == (self.mini_batch_size, 1)
        assert action_log_probs_beta.shape == (self.mini_batch_size, 1)
        assert values.requires_grad
        assert action_log_probs_cash.requires_grad
        assert action_log_probs_beta.requires_grad
        assert dist_entropy.requires_grad

        # [TODO] Implement policy loss
        ratio = torch.exp(action_log_probs_cash-old_action_log_probs_cash_batch)
        advantage1 = ratio*adv_targ
        advantage2 = torch.clamp(ratio, 1-self.clip_param, 1+self.clip_param)*adv_targ
        policy_loss_cash = -(torch.min(advantage1, advantage2)).mean()

        ratio = torch.exp(action_log_probs_beta-old_action_log_probs_beta_batch)
        advantage1 = ratio*adv_targ
        advantage2 = torch.clamp(ratio, 1-self.clip_param, 1+self.clip_param)*adv_targ
        policy_loss_beta = -(torch.min(advantage1, advantage2)).mean()

        # [TODO] Implement value loss
        value_loss = F.mse_loss(return_batch, values)

        # This is the total loss
        loss = policy_loss_cash + policy_loss_beta + self.value_loss_weight * value_loss - \
               self.entropy_loss_weight * dist_entropy

        return loss, policy_loss_cash+policy_loss_beta, value_loss, dist_entropy
