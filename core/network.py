...
class ActorCritic(nn.Module):
    def __init__(self, input_shape, num_actions):
        ...
        self.linear = nn.Linear(input_shape[0]*input_shape[1], 16)

        # self.linear2 = nn.Linear(16, 16)

        init_ = lambda m: self.layer_init(
            m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0))
        self.critic_linear = init_(nn.Linear(16, 1))

        init_ = lambda m: self.layer_init(
            m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), gain=0.01
        )
        self.actor_linear = init_(nn.Linear(16, num_actions))

        self.actor_linear_beta = init_(nn.Linear(16, 5))
    ...
    def forward(self, inputs):
        ...
        logits_cash = self.actor_linear(x)
        logits_beta = self.actor_linear_beta(x)
        return logits_cash, logits_beta, value
