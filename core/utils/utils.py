...
def step_envs(cpu_actions, envs, env_id, episode_rewards, episode_values, frame_stack_tensor,
    ...
    if done:
        reward_recorder.append(episode_rewards[env_id].copy())
        episode_values.append(envs.portfolio.value)
        length_recorder.append(200)
        total_episodes += 1
        masks = np.array(1)
    else:
        masks = np.array(1)
    ...
    if test:
        ...
        frame_stack_masks = masks.view(-1, 1, 1)
    # If in multiple pong mode, we suppose only the first observation is used to
    # train agent.

    frame_stack_tensor.update(obs, env_id, frame_stack_masks)

    return obs, reward, done, masks, total_episodes, total_steps, \
...
def evaluate(trainer, eval_envs, baseline_trainer, frame_stack, num_episodes=10, seed=0):
    ...
    frame_stack_tensor = FrameStackTensor(
        num_episodes, eval_envs.observation_space.shape, frame_stack,
        trainer.device
    )

    def get_action(frame_stack_tensor, env_id):
        obs = frame_stack_tensor.get(env_id)
        if isinstance(obs, np.ndarray):
            obs = torch.from_numpy(obs).to(trainer.device)
        with torch.no_grad():
            # actions = trainer.compute_action(obs, deterministic=True)[1]
            actions_cash = trainer.compute_action(obs)[1]
            actions_beta = trainer.compute_action(obs)[3]
    ...
    episode_rewards = np.zeros([num_episodes, 1], dtype=np.float)
    total_steps = 0
    total_episodes = 0
    eval_envs.seed(seed)
    for env_id in range(num_episodes):
        obs = eval_envs.reset(test=True)
        frame_stack_tensor.update(obs, env_id)
        while True:
            obs, reward, done, masks, total_episodes, total_steps, \
            episode_rewards, episode_values = step_envs(
                get_action(frame_stack_tensor, env_id), eval_envs, env_id, episode_rewards,
                episode_values, frame_stack_tensor, reward_recorder, episode_length_recorder,
                total_steps, total_episodes, trainer.device, frame_stack == 1)
            if done:
                break
            # if total_episodes >= num_episodes:
            #     break
    return reward_recorder, episode_length_recorder, episode_values
...
class FrameStackTensor:
    def __init__(self, num_envs, obs_shape, frame_stack, device):
        ...
        self.num_channels = 1
        self.obs_shape = (1 * frame_stack, *obs_shape[0:])
        self.current_obs = torch.zeros(num_envs, *self.obs_shape, device=device,
                                       dtype=torch.float)
        self.mask_shape = [1] * (self.current_obs.dim()-1)
        self.mask_shape[0] = -1
        self.device = device

    def update(self, obs, env_id, mask=None):
        ...
        if mask is not None:
            ...
            self.current_obs[env_id] *= mask

        if env_id == 0:
            self.current_obs = self.current_obs.roll(shifts=-self.num_channels, dims=1)

        obs = torch.from_numpy(obs.astype(np.float32)).to(self.device)
        self.current_obs[env_id, -self.num_channels:] = obs
    ...
    def get(self, env_id):
        return self.current_obs[env_id]
